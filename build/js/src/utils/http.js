...
function exponentialBackoff(attempt, base, cap) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (shouldRetryStatus(status) && attempt < retry.retries) {
                    const delay = exponentialBackoff(attempt, retry.baseDelayMs, retry.maxDelayMs);
                    attempt++;
                    await sleep(delay);
                    continue;
                }
            ...
            try {
                ...
                return err(error.parse("Invalid JSON in response", { cause: parseError }));
            }
        }
        catch (errorObject) {
            clearTimeout(timeoutId);
            const isAbort = (errorObject instanceof DOMException &&
                errorObject.name === "AbortError") ||
                errorObject?.name === "AbortError";
            const baseError = isAbort
                ? error.timeout(timeoutMs)
                : error.network(errorObject?.message ?? "Network");
